		...
		try {
			...
			const aiRes = await fetch('/api/ai/engine', {
				...
				body: JSON.stringify({
					...
					context: {
						...
						costOfInaction: solveData.triggerQuestions?.[1]?.answer ?? '',
						successKPI: solveData.triggerQuestions?.[2]?.answer ?? '',
						existingTool: solveData.triggerQuestions?.[3]?.answer ?? '',
						budget: solveData.triggerQuestions?.[5]?.answer ?? '',
			...
			for (const fs of fitScored.slice(0, 5)) {
				...
				merged.push({
					...
					whyFits: fs.signals?.slice(0, 2).join('. ') || 'Matches your profile',
